...
TEST(ErrorTest, ReturnIfErrorNoError) {
  auto result = []() -> ErrorOr<Success> {
    PXC_RETURN_IF_ERROR(ErrorOr<Success>(Success()));
    PXC_RETURN_IF_ERROR(ErrorOr<Success>(Success()));
    return Success();
  }();
...
TEST(ErrorTest, ReturnIfErrorHasError) {
  auto result = []() -> ErrorOr<Success> {
    PXC_RETURN_IF_ERROR(ErrorOr<Success>(Success()));
    PXC_RETURN_IF_ERROR(ErrorOr<Success>(Error("error")));
    return Success();
  }();
...
TEST(ErrorTest, AssignOrReturnNoError) {
  auto result = []() -> ErrorOr<int> {
    PXC_ASSIGN_OR_RETURN(int a, ErrorOr<int>(1));
    PXC_ASSIGN_OR_RETURN(const int b, ErrorOr<int>(2));
    int c = 0;
    PXC_ASSIGN_OR_RETURN(c, ErrorOr<int>(3));
    return a + b + c;
  }();
...
TEST(ErrorTest, AssignOrReturnHasDirectError) {
  auto result = []() -> ErrorOr<int> {
    PXC_RETURN_IF_ERROR(ErrorOr<int>(Error("error")));
    return 0;
  }();
  ASSERT_FALSE(result.ok());
}
...
TEST(ErrorTest, AssignOrReturnHasErrorInExpected) {
  auto result = []() -> ErrorOr<int> {
    PXC_ASSIGN_OR_RETURN(int a, ErrorOr<int>(Error("error")));
    return a;
  }();
  ASSERT_FALSE(result.ok());
  EXPECT_EQ(result.error().message(), "error");
